...
sys.path.append('drive1/My Drive/AA/voxelmorph-master/ext/neuron')
sys.path.append('drive1/My Drive/AA/voxelmorph-master/ext/pynd-lib')
sys.path.append('drive1/My Drive/AA/voxelmorph-master/ext/pytools-lib')
...
# other vm functions
import losses
def ResUnet1(vol_size, enc_nf, dec_nf, full_size=True, src=None, tgt=None, src_feats=1, tgt_feats=1):
...
def ResUnet(vol_size, full_size=True, src=None, tgt=None, src_feats=1, tgt_feats=1):
    ...
    input_segBN = BatchNormalization()(x_in)
    ...
    up7_cm = Conv2D(256, 3, activation='relu', padding='same', kernel_initializer='he_normal')(UpSampling2D(size=(2, 2))(conv5_spect)) #24x24
    ...
    model = Model(inputs=[src, tgt], outputs=[out])
...
def miccai2018_net(vol_size, enc_nf, dec_nf, int_steps=7, use_miccai_int=False, indexing='ij', bidir=False, vel_resize=1/2):
    ...
    # unet_model =ResUnet(vol_size, full_size=False)
    [src, tgt] = unet_model.inputs
